...
import { BuildKhaltiExamPayload, KhaltiExamPaymentPayload } from '@/lib/helper/khalti'
...
const MakePaymentPage = () => {
    ...
    const selectedCoursesIds = searchParams.get("courses")?.split(',').map(Number) ?? [];
    const {
        data: semesterData,
    ...
    const courses = semesterData?.courses.filter(course =>
        selectedCoursesIds.includes(course.id)
    ) ?? [];

    const khaltiMutation = useMutation({
        mutationFn: async (payload: KhaltiExamPaymentPayload) => await initiateKhalti(payload),
        onSuccess: data => {
            console.log(data)
            ...
            <div className='flex flex-col border p-4 gap-4 w-[1000px]'>
                ...
                <div className="border-neutral w-full overflow-x-auto rounded-md border-x border-t text-base">
                    <table className="min-w-full border-collapse">
                        ...
                        <tbody>
                            {semesterData && semesterData.courses &&
